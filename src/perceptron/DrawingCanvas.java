...
class DrawingCanvas extends Canvas {
	 ...
	 * and then call the display() method on the Canvas to update the visible
	...
	public void drawLine(int x1, int y1, int x2, int y2, boolean redraw) {
		...
		if (redraw) {
			display(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1) + 1, Math.abs(y2 - y1) + 1);
		}
	}
	...
	public void invertArc(int x, int y, int width, int height, int startAngle, int arcAngle, boolean redraw) {
		imgGraphic.setXORMode(Color.white);
		imgGraphic.drawArc(x, y, width - 1, height - 1, startAngle, arcAngle);
		imgGraphic.setPaintMode();
		if (redraw)
	...
	public void eraseArc(int x, int y, int width, int height, int startAngle, int arcAngle, boolean redraw) {
		Color save = imgGraphic.getColor();
		imgGraphic.setColor(Color.white);
		imgGraphic.drawArc(x, y, width - 1, height - 1, startAngle, arcAngle);
		imgGraphic.fillArc(x, y, width, height, startAngle, arcAngle);
	...
	/* from Image, scaled */
	public void drawImage(Image img, int x, int y, int width, int height, boolean redraw) {
		imgGraphic.drawImage(img, x, y, width, height, this.getBackground(), this);
		if (redraw)
			display();
	}
